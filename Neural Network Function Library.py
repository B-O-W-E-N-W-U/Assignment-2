...
# Initialize weight + first and second moment matrices (Adam) for all layers
def parameter_generate(layer, train):
    weight, mAd, vAd = {}, {}, {}
    input_dim = np.shape(train)[0] # Dimension of training input
    for i in range(len(layer)):
        if i == 0:
            weight[i] = np.random.uniform(-1, 1, [layer[i], input_dim]) # Weight matrix for the input layer
        else:
            weight[i] = np.random.uniform(-1, 1, [layer[i], layer[i - 1] + 1]) # Weight matrices for the hidden and the output layers
    for i in range(len(weight)):
        mAd[i] = vAd[i] = np.zeros(weight[i].shape) # Initialize first and second order moments to zero
...
def forward_propagation(weight, train, act_func, column):
    ...
    for i in range(len(weight)):
        if i == 0: # Net activation v for the first hidden layer
            v[i] = weight[i] @ np.reshape(train[:, column], [input_dim, 1]) # Reshape v explicitly to maintain the vector property of the training input
        else:
            v[i] = weight[i] @ np.reshape(xo[i - 1],[len(xo[i - 1]), 1]) #'@': matrix multiplication
        if i != len(weight) - 1: # Activation xo for the hidden layers
            xo[i] = np.insert(activation(v[i], act_func[i]), 0, 1) # Include the bias '1' in every activation
        else: # Activation for the output layer
            xo[i] = activation(v[i], act_func[i]) # Remove the bias '1' for the output layer
    return v, xo 
...
# Model predictions on training and test set
def prediction(weight, set, settarget, act_func):
    ...
    y = np.empty((output_dim, 1)) # Initialize model predictions with 2D empty array with no bias
    ...
    y = np.reshape(np.delete(y,0, axis=1),[output_dim, np.shape(set)[1]]) # Remove the initial empty column
    return y
...
def regression(train, traintarget, test, testtarget, layer, act_func, eta, beta1, beta2, eps, algorithm, performance, max_epoch):
    weight, mAD, vAD = parameter_generate(layer, train)                                                                                                                                                        
    weight, costtrainrecord, costtestrecord, epochrecord = train_network(weight, mAD, vAD, layer, train, traintarget, test, testtarget, eta, beta1, beta2, eps, act_func, algorithm, performance, max_epoch)    
    y = prediction(weight, test, testtarget, act_func)                                                                                                                                                          
    return y, costtrainrecord, costtestrecord, epochrecord

# Classification
def classification(train, trainlabel, test, testlabel, layer, act_func, eta, beta1, beta2, eps, algorithm, performance, max_epoch):                                                          
    weight, mAD, vAD = parameter_generate(layer, train)                                                                                                                                                        
    weight, costtrainrecord, costtestrecord, epochrecord = train_network(weight, mAD, vAD, layer, train, trainlabel, test, testlabel, eta, beta1, beta2, eps, act_func, algorithm, performance, max_epoch)      
    y = prediction(weight, test, testlabel, act_func)                                                                                                                                                          
    return y, costtrainrecord, costtestrecord, epochrecord
